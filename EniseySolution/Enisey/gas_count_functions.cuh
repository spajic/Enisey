// Содержимое этого файла теперь живёт в functions_gas.h
/*#pragma once
...
inline double FindTPseudoCritical(double den_sc, double co2, double n2)
{
	return 88.25 * (0.9915 + 1.759 * den_sc - co2 - 1.681*n2); // [Кельвины]	
};

// Найти псевдокритическое давление [МПа]
inline double FindPPseudoCritical(double den_sc, double co2, double n2)
{
	return 2.9585 * (1.608 - 0.05994*den_sc - co2 - 0.392*n2); // [МегаПаскали]
};

// Фактор сжимаемости при с.у. [б.р.]
inline double FindZStandartConditions(double den_sc, double co2, double n2)
{
	return 1 - pow(0.0741*den_sc-0.006-0.063*n2-0.0575*co2, 2); // [б.р.]
};

// Газовая постоянная при с.у. [Джоуль / (Моль*Кельвин)]
inline double FindRStandartConditions(double den_sc)
{
...
inline double FindPReduced(double p_work, double p_pseudo_critical)
{
	return p_work / p_pseudo_critical; // [б.р.]
};
	
inline double FindTReduced(double t_work, double t_pseudo_critical)
{
	return t_work / t_pseudo_critical; // [б.р.]
};

// Теплоёмкость при р.у. [Дж/(кг*К)]
inline double FindC(double t_reduced, double p_reduced, double r_standart_conditions)
{
...
inline double FindDi(double p_reduced, double t_reduced)
{
...
inline double FindMju(double p_reduced, double t_reduced)
{
...
inline double FindZ(double p_reduced, double t_reduced)
{
...
inline double FindRo(double den_sc, double p_work, double t_work, double z)
{
...
// Число Рейнольдса
inline double FindRe(double q, double den_sc, double mju, double d_inner)
{
...
inline double FindLambda(double re, double d_inner, double roughness_coefficient, double hydraulic_efficiency_coefficient)
{
